    ...
    firebase.database().ref('Questions').once("value", function(data){
        ...
        for(var property in data){
            if(data.hasOwnProperty(property)){
                ...
                questions[qNum].marks = data[property].marks
...
function addRec(id){
    var marksVal = id.parentNode.parentNode.childNodes[5].childNodes[1].value;
    ...
    if(!editingRec) var key = firebase.database().ref('Questions').push().key 
...
function editRec(id){
    if(!addingRec){
        ...
        htmlDesign += '<li style="display: flex; justify-content: center;">'
        htmlDesign += `<button class="btn btn-success liBtnAdd fa fa-check" id="${key}" onclick="addRec(this)"></button>`
        htmlDesign += `<button class="btn btn-danger liBtnCancel fa fa-times" id="${key}" onclick="cancelUpdateRec(this)"></button>`
        htmlDesign += `<li style="display: flex;"><h3>Marks:&nbsp</h3><input class="form-control w-50" value="${marksVal}"></li>`
        htmlDesign += '</li>'
        htmlDesign += '</ul>'
        htmlDesign += '</li>'
